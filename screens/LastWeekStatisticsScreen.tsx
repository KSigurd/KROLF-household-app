...
const LastWeekStatisticsScreen: FC = () => {
  ...
  const blaa = statisticsState.map((chore) => {
    if (
      chore.completedChores.find(
        (completedChore) => completedChore.completedChores.length != 0
      )
    ) {
      return chore;
    }
  ...
  //   const activeHouseholdId = "1";
    ...
    const chores =useAppSelector(
      (state) =>  state.chore.chores);
    console.log(chores)
  ...
  //   }
  // }
  const statisticsData: ChoreStatisticsDTO = {
    choreTitle: "Totalt",
    points: 0,

  }
  for(const item of statisticsState){
  ...
  return (
    <View style={styles.root}>
      ...
      <View style={styles.circles}>
        ...
        .map(s => {
          const data = s.completedChores.map((cc) => ({
              avatarId: cc.avatarId,
              // totalPoints: (data.points * dataObject.completedChores.length),
              totalPoints: 1,
          }))
          return (
          <StatisticCircle
...
export default LastWeekStatisticsScreen;
